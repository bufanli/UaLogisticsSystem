(function ($)
{
	...
	jqueryUtil.chgSkin=function(selectId,cookiesColor){
      ...
      //高级查询
        jqueryUtil.gradeSearch=function($dg,formId) {
        	$dg.datagrid('reload',jqueryUtil.serializeObject($(formId)));
		};
		//添加
		jqueryUtil.addRow = function($dg,title,url,width,height, formId){
		...
		//修改
		jqueryUtil.updRow = function($dg,title,url,width,height,formId){
		...
		//删除
		jqueryUtil.delRows = function($dg,url,method){
								...
								success: function(data){
									   if(data.result=="success"){
			                               ...
			                               $.messager.show({
					                    	...
					                    	$.messager.confirm('登录超时，请重新登录！', function (confirm) {
		                					if (confirm) {
		                						if (window != top) {
													top.location.href = data.baseurl+'/admin/login';
												}
		                					}});
					                    }else{
		...
		jqueryUtil.form=function($dg,url,type){
			$dg.form({
				url :url,
				onSubmit : function() {
					$.messager.progress({
