(function ($)
{
	...
	//修改ajax默认设置
		...
		//IE检测
			easyuiUtil.isLessThanIe8 = function() {
		...
		//序列化表单到对象
		easyuiUtil.serializeObject = function(form) {
	...
	//获取随机时间
	 easyuiUtil.getRandTime=function(){
	...
	//切换皮肤
	easyuiUtil.chgSkin=function(selectId,cookiesColor){
            ...
            function docchgskin(dc,selectId,cookiesColor){
						removejscssfile(dc,cookiesColor, "css");
                        createLink(dc,selectId);
        	}
      ...
      //高级查询
        easyuiUtil.gradeSearch=function($dg,formId) {
        	$dg.datagrid('reload',easyuiUtil.serializeObject($(formId)));
		};

		//添加
		easyuiUtil.openAddDialog = function($dg, title, url, width, height, formId) {
		    $.modalDialog({
		        title: title,
		        width: width,
		        height: height,
		        ...
		        buttons: [{
		            ...
		            iconCls: 'icon-ok',
		            handler: function() {
		                $.modalDialog.openner = $dg; //因为添加成功之后，需要刷新这个treegrid，所以先预定义好
		                var f;
		                if (formId) {
		                    f = $.modalDialog.handler.find("#" + formId);
		                } else {
		                    f = $.modalDialog.handler.find("#form");
		                }
		                f.submit();
		                //$dg.datagrid("reload");
		            }
		...
		//修改
		easyuiUtil.openUpdateDialog = function($dg, title, url, width, height, formId) {
		    var idField = $dg.datagrid('options').idField || 'id';
		    var row = $dg.datagrid('getSelected');
		    if (row) {
		        url = url + eval("row." + idField);
		        $.modalDialog({
		            title: title,
		            width: width,
		            height: height,
		...
		//删除
		easyuiUtil.delRows = function($dg, url, method) {
		        ...
		        function(r) {
		            if (r) {
		                ...
		                $(selecteds).each(function(i, item) {
		                    count++;
		                    checkedList.push(eval("item." + idField));
		                });
		                $.ajax({
		                    dataType: 'json',
		                    url: url,
		...
		easyuiUtil.form = function($dg, url, type, cbFuncs) {
		    $dg.form({
		        url: url,
		        onSubmit: function() {
		        	if (cbFuncs.beforeSubmit != undefined && $.isFunction(cbFuncs.beforeSubmit)) {
		        		if (!cbFuncs.beforeSubmit()) {
		        			return false;
		        		}
		        	}
		            $.messager.progress({
